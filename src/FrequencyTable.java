		...
		for(int i = 0; i  < Compose.database.length()-1; i++){
			if(Compose.database.charAt(i) == note){
				if(notes.contains(Compose.database.charAt(i+1))){
					freqs.set(notes.indexOf(Compose.database.charAt(i+1)), freqs.get(notes.indexOf(Compose.database.charAt(i+1)))+1);
				}else{
...
//		return notes.get( maxIndices[(int) (Math.random()*maxIndices.length)]);
